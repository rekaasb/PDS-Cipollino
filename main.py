...
# ============================================================================
# 2. ФУНКЦИЯ ГЕНЕРАЦИИ СИНТЕТИЧЕСКОГО КУБА ДАННЫХ (Плоская 2D ФАР)
# ============================================================================
def generate_multi_target_cube(M, K, L, targets, SNR_dB=10):
    ...
    for t_id, t_info in targets.items():
        ...
        for l in range(L):
            # Использовали глобальную константу T_PRI заглавными буквами
            t = l * T_PRI
...
def main():
    ...
    # Использование глобальных констант, чтобы избежать UnboundLocalError
    global M, K, L
